...
public class TimeCommand extends VanillaCommand {
    ...
    public boolean execute(CommandSender sender, String currentAlias, String[] args) {
        if (args.length < 2) {
            sender.sendMessage(Bukkit.getServer().getNegativeColor() + "Incorrect usage. Correct usage:\n" + usageMessage);
            return false;
        }
        ...
        if (args[0].equals("set")) {
            if (!sender.hasPermission("bukkit.command.time.set")) {
                sender.sendMessage(Bukkit.getServer().getNegativeColor() + "You don't have permission to set the time");
                return true;
            }
