	...
	public boolean execute(CommandSender sender, String currentAlias, String[] args) {
		if (args.length == 0) {
			...
			if (!found && sender.isOp()) {
				sender.sendMessage(VersionCommand.PRIMARY_COLOR + "This server is not running any plugin by that name.");
	...
	private void describeToSender(Plugin plugin, CommandSender sender) {
		...
		if (!desc.getAuthors().isEmpty()) {
			if (desc.getAuthors().size() == 1) {
				sender.sendMessage(Bukkit.getServer().getFirstColor() + "Author: " + getAuthors(desc));
			} else {
				sender.sendMessage(Bukkit.getServer().getFirstColor() + "Authors: " + getAuthors(desc));
			}
		}
	}
