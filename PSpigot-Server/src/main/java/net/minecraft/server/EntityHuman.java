...
public abstract class EntityHuman extends EntityLiving {
    ...
    public ExecutorService hitDetection = Executors.newFixedThreadPool(Bukkit.getServer().getHitDetection(), new ThreadFactoryBuilder().setNameFormat("Async Hit Detection").build());
    ...
    public boolean sprintKnockBack = false;
        ...
        if (entity.aD()) {
            if (!entity.l(this)) {
                Runnable runnable = new BukkitRunnable() {
                    public void run() {
                        float f = (float) EntityHuman.this.getAttributeInstance(GenericAttributes.ATTACK_DAMAGE).getValue();
                        byte b0 = 0;

                        float f1 = 0.0F;

                        if (entity instanceof EntityLiving) {
                            f1 = EnchantmentManager.a(EntityHuman.this.bA(), ((EntityLiving) entity).getMonsterType());
                        } else {
                            f1 = EnchantmentManager.a(EntityHuman.this.bA(), EnumMonsterType.UNDEFINED);
                        }

                        int i = b0 + EnchantmentManager.a(EntityHuman.this);

                        if (EntityHuman.this.isSprinting()) {
                            ++i;
                        }

                        if (f > 0.0F || f1 > 0.0F) {
                            boolean flag = !world.paperSpigotConfig.disablePlayerCrits && EntityHuman.this.fallDistance > 0.0F
                                    && !EntityHuman.this.onGround && !EntityHuman.this.k_() && !EntityHuman.this.V() && !EntityHuman.this.hasEffect(MobEffectList.BLINDNESS)
                                    && EntityHuman.this.vehicle == null && entity instanceof EntityLiving; // PaperSpigot
                            ...
                            f += f1;
                            boolean flag1 = false;
                            int j = EnchantmentManager.getFireAspectEnchantmentLevel(EntityHuman.this);
                            ...
                            if (entity instanceof EntityLiving && j > 0 && !entity.isBurning()) {
                                ...
                                // enchanted item
                                EntityCombustByEntityEvent combustEvent = new EntityCombustByEntityEvent(EntityHuman.this.getBukkitEntity(),
                            ...
                            boolean flag2 = entity.damageEntity(DamageSource.playerAttack(EntityHuman.this), f);

                            KnockBack profile = EntityHuman.this.getKnockbackProfile() == null ? Bukkit.getServer().getActiveKnockback() : EntityHuman.this.getKnockbackProfile();
                            boolean combo = !(entity.getBukkitEntity() instanceof CraftPlayer) || (profile.isComboMode());

                            if (flag2) {
                                if (((entity instanceof EntityPlayer)) && (entity.velocityChanged)) {
                                    EntityPlayer attackedPlayer = (EntityPlayer)entity;
                                    Player damager = (Player)getBukkitEntity();
                                    Player damaged = (Player)entity.getBukkitEntity();
                                    Vector kb;
                                    if (!combo) {
                                        double x = profile.getHor();
                                        double y = profile.getVer();
                                        double z = profile.getHor();

                                        if (damaged.isOnGround()) {
                                            x = profile.getHorGround() * profile.getHor();
                                            y = profile.getVerGround() * profile.getVer();
                                            z = profile.getHorGround() * profile.getHor();
                                        }

                                        if (EntityHuman.this.sprintKnockBack) {
                                            x = profile.getHorSprint() * profile.getHor();
                                            y = profile.getVerSprint() * profile.getVer();
                                            z = profile.getHorSprint() * profile.getHor();

                                            sprintKnockBack = false;
                                        }

                                        if (isSprinting())  {
                                            x = profile.getHorWtap() * profile.getHor();
                                            y = profile.getVerWtap() * profile.getVer();
                                            z = profile.getHorWtap() * profile.getHor();
                                        ...
                                        kb = damaged.getLocation().toVector().subtract(damager.getLocation().toVector()).normalize();
                                        kb.setX(kb.getX() * x);
                                        kb.setY(0.35D * y);
                                        kb.setZ(kb.getZ() * z);
                                        ...
                                        kb = new Vector(attackedPlayer.motX * profile.getComboHor(), attackedPlayer.motY * profile.getComboIncrease(), attackedPlayer.motZ * profile.getComboHor());
                                        if (damaged.getLocation().getBlockY() - damager.getLocation().getBlockY() >= profile.getComboLimit()) {
                                            if (damaged.getLocation().subtract(0.0D, profile.getComboLimit(), 0.0D).getBlock().getType() == org.bukkit.Material.AIR) {
                                                kb.setY(attackedPlayer.motY - profile.getComboReducer());
                                            }
                                        }
                                        entity.motX = kb.getX();
                                        entity.motY = kb. getY();
                                    ...
                                    EntityHuman.this.world.getServer().getPluginManager().callEvent(new PlayerVelocityEvent(attackedPlayer.getBukkitEntity(), kb));
                                    attackedPlayer.playerConnection.sendPacket(new PacketPlayOutEntityVelocity(attackedPlayer.getBukkitEntity().getEntityId(), kb.getX(), kb.getY(), kb.getZ()));
                                ...
                                if (flag) {
                                    EntityHuman.this.b(entity);
                                }

                                if (f1 > 0.0F) {
                                    EntityHuman.this.c(entity);
                                }

                                if (f >= 18.0F) {
                                    EntityHuman.this.b((Statistic) AchievementList.F);
                                }

                                EntityHuman.this.p(entity);
                                if (entity instanceof EntityLiving) {
                                    EnchantmentManager.a((EntityLiving) entity, (Entity) EntityHuman.this);
                                }

                                EnchantmentManager.b(EntityHuman.this, entity);
                                ItemStack itemstack = EntityHuman.this.bZ();
                                ...
                                if (itemstack != null && object instanceof EntityLiving) {
                                    itemstack.a((EntityLiving) object, EntityHuman.this);
                                    // CraftBukkit - bypass infinite items; <= 0 -> == 0
                                    if (itemstack.count == 0) {
                                        EntityHuman.this.ca();
                                    }
                                ...
                                if (entity instanceof EntityLiving) {
                                    EntityHuman.this.a(StatisticList.w, Math.round(f * 10.0F));
                                    if (j > 0) {
                                        // CraftBukkit start - Call a combust event when somebody hits with a fire
                                        // enchanted item
                                        EntityCombustByEntityEvent combustEvent = new EntityCombustByEntityEvent(EntityHuman.this.getBukkitEntity(), entity.getBukkitEntity(), j * 4);
                                        org.bukkit.Bukkit.getPluginManager().callEvent(combustEvent);
                                ...
                                EntityHuman.this.applyExhaustion(world.spigotConfig.combatExhaustion); // Spigot - Change to use
                                // configurable value
                            } else if (flag1) {
                                entity.extinguish();
                            }
                ...
                this.hitDetection.submit(runnable);
            }
        }
    }
