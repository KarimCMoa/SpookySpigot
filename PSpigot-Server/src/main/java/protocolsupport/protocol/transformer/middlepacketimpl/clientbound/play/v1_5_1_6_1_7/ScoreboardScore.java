...
extends MiddleScoreboardScore<RecyclableCollection<PacketData>> {
    @Override
    public RecyclableCollection<PacketData> toData(ProtocolVersion version) {
        PacketData serializer = PacketData.create(ClientBoundPacket.PLAY_SCOREBOARD_SCORE_ID, version);
        serializer.writeString(Utils.clampString(this.name, 16));
        serializer.writeByte(this.mode);
        if (this.mode != 1) {
            serializer.writeString(this.objectiveName);
