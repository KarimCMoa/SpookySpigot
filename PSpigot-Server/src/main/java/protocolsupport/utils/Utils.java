...
public class Utils {
    ...
    public static void setStaticFinalField(Field field, Object newValue) throws NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException {
        Utils.setAccessible(Field.class.getDeclaredField("modifiers")).setInt(field, field.getModifiers() & -17);
        Utils.setAccessible(Field.class.getDeclaredField("root")).set(field, null);
        Utils.setAccessible(Field.class.getDeclaredField("overrideFieldAccessor")).set(field, null);
        Utils.setAccessible(field).set(null, newValue);
    }
    ...
    public static List<int[]> splitArray(int[] array, int limit) {
        ...
        int count = Utils.getSplitCount(array.length, limit);
        int copied = 0;
