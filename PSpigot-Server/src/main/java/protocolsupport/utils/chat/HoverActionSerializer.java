...
JsonSerializer<HoverAction> {
    ...
    public JsonElement serialize(HoverAction action, Type type, JsonSerializationContext ctx) {
        ...
        object.add("value", (JsonElement)(action.getType() == HoverAction.Type.SHOW_TEXT ? ctx.serialize((Object)ChatAPI.fromJSON(action.getValue())) : new JsonPrimitive(action.getValue())));
        return object;
    }
}
